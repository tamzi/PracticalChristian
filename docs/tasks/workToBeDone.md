...
- [x] STORY: Validation
  ...
  - [x] TASK: Confirm there are no Cursor, Codex, or Claude Code project-local adapters yet.
  - [x] TASK: Test Firebender skill adapter resolution and agent registration paths from `.ai` and `.firebender`.
  - [x] TASK: Verify `.firebender/skills` symlinks are Git-tracked and resolve to repository-local `.ai/skills` directories.
